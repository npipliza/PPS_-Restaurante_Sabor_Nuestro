...
export class LoginPage implements OnInit {
  ...
  Login(){
    ...
    .then( async(res) => {    
      const user = (await this.userSrv.getUserByUid(res.user.uid).toPromise()).data();
  ...
  testUser(accountNumber: number){
    switch (accountNumber) {
      case 1:
        this.loginForm.controls['email'].setValue('jefe@gmail.com') 
        this.loginForm.controls['password'].setValue('qweqwe')
        break;
      case 2:
